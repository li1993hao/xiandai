<?php /* Smarty version Smarty-3.1.14, created on 2014-09-22 11:06:07
         compiled from "app/tpl/activityjobbulletin/index.htm" */ ?>
<?php /*%%SmartyHeaderCode:1185880583541f921fd255a3-32874014%%*/if(!defined('SMARTY_DIR')) exit('no direct access allowed');
$_valid = $_smarty_tpl->decodeProperties(array (
  'file_dependency' => 
  array (
    '8b81a956dd7177c056cc65aeee2eed7bfae4a157' => 
    array (
      0 => 'app/tpl/activityjobbulletin/index.htm',
      1 => 1399630038,
      2 => 'file',
    ),
  ),
  'nocache_hash' => '1185880583541f921fd255a3-32874014',
  'function' => 
  array (
  ),
  'variables' => 
  array (
    'web_url' => 0,
    'perid' => 0,
    'haspre' => 0,
    'hasnext' => 0,
    'bulletin' => 0,
  ),
  'has_nocache_code' => false,
  'version' => 'Smarty-3.1.14',
  'unifunc' => 'content_541f921fe810c8_41194635',
),false); /*/%%SmartyHeaderCode%%*/?>
<?php if ($_valid && !is_callable('content_541f921fe810c8_41194635')) {function content_541f921fe810c8_41194635($_smarty_tpl) {?><?php if (!is_callable('smarty_modifier_date_format')) include '/Users/haoli/Desktop/www/wyjob/been/Smarty/plugins/modifier.date_format.php';
if (!is_callable('smarty_modifier_truncate')) include '/Users/haoli/Desktop/www/wyjob/been/Smarty/plugins/modifier.truncate.php';
if (!is_callable('smarty_function_page')) include '/Users/haoli/Desktop/www/wyjob/been/View/plugins/function.page.php';
?><?php $_smarty_tpl->tpl_vars['action'] = new Smarty_variable("Activityjobbulletin/index", null, 0);?>
<?php $_smarty_tpl->tpl_vars['title'] = new Smarty_variable("天外职刊&nbsp;O<em>ccupation&nbsp;Publications</em>", null, 0);?>
<?php $_smarty_tpl->tpl_vars['phonetitle'] = new Smarty_variable("天外职刊", null, 0);?>
<?php $_smarty_tpl->tpl_vars['color'] = new Smarty_variable('red', null, 0);?>
<?php $_smarty_tpl->tpl_vars['topid'] = new Smarty_variable('jobinfo_top', null, 0);?>
<?php $_smarty_tpl->tpl_vars['westid'] = new Smarty_variable('jobinfo_west', null, 0);?>
<?php echo $_smarty_tpl->getSubTemplate ('header.html', $_smarty_tpl->cache_id, $_smarty_tpl->compile_id, null, null, array(), 0);?>

    <link type="text/css" rel="stylesheet" href="<?php echo $_smarty_tpl->tpl_vars['web_url']->value;?>
/common/app/css/common/content.css" />
<link type="text/css" rel="stylesheet" href="<?php echo $_smarty_tpl->tpl_vars['web_url']->value;?>
/common/app/css/common/middle-right.css" /> 
    
    <div id="middle">
    	<div id="middle_left">
            <div id="work">
            	
            	
            	<div class="middle_left_title">
            		<div class="middle_left_title_left">
            			第<?php echo $_smarty_tpl->tpl_vars['perid']->value['ap_number'];?>
期
            		</div>
            		<div class="middle_left_title_right">
            			<?php if ($_smarty_tpl->tpl_vars['haspre']->value){?>
            			<a href="<?php echo $_smarty_tpl->tpl_vars['web_url']->value;?>
/index.php/activityjobbulletin/index/do/pre/id/<?php echo $_smarty_tpl->tpl_vars['perid']->value['ap_id'];?>
"  title = "上一期"><上一期</a>
            			<?php }else{ ?>
            			上一期
            			<?php }?>
            			|
            			<?php if ($_smarty_tpl->tpl_vars['hasnext']->value){?>
            			<a href="<?php echo $_smarty_tpl->tpl_vars['web_url']->value;?>
/index.php/activityjobbulletin/index/do/next/id/<?php echo $_smarty_tpl->tpl_vars['perid']->value['ap_id'];?>
"  title = "下一期" >下一期></a>
            			<?php }else{ ?>
            			下一期
            			<?php }?>
            		</div>
            	</div>
            
                <?php if (isset($_smarty_tpl->tpl_vars['smarty']->value['section']['n'])) unset($_smarty_tpl->tpl_vars['smarty']->value['section']['n']);
$_smarty_tpl->tpl_vars['smarty']->value['section']['n']['name'] = 'n';
$_smarty_tpl->tpl_vars['smarty']->value['section']['n']['loop'] = is_array($_loop=$_smarty_tpl->tpl_vars['bulletin']->value['list']) ? count($_loop) : max(0, (int)$_loop); unset($_loop);
$_smarty_tpl->tpl_vars['smarty']->value['section']['n']['show'] = true;
$_smarty_tpl->tpl_vars['smarty']->value['section']['n']['max'] = $_smarty_tpl->tpl_vars['smarty']->value['section']['n']['loop'];
$_smarty_tpl->tpl_vars['smarty']->value['section']['n']['step'] = 1;
$_smarty_tpl->tpl_vars['smarty']->value['section']['n']['start'] = $_smarty_tpl->tpl_vars['smarty']->value['section']['n']['step'] > 0 ? 0 : $_smarty_tpl->tpl_vars['smarty']->value['section']['n']['loop']-1;
if ($_smarty_tpl->tpl_vars['smarty']->value['section']['n']['show']) {
    $_smarty_tpl->tpl_vars['smarty']->value['section']['n']['total'] = $_smarty_tpl->tpl_vars['smarty']->value['section']['n']['loop'];
    if ($_smarty_tpl->tpl_vars['smarty']->value['section']['n']['total'] == 0)
        $_smarty_tpl->tpl_vars['smarty']->value['section']['n']['show'] = false;
} else
    $_smarty_tpl->tpl_vars['smarty']->value['section']['n']['total'] = 0;
if ($_smarty_tpl->tpl_vars['smarty']->value['section']['n']['show']):

            for ($_smarty_tpl->tpl_vars['smarty']->value['section']['n']['index'] = $_smarty_tpl->tpl_vars['smarty']->value['section']['n']['start'], $_smarty_tpl->tpl_vars['smarty']->value['section']['n']['iteration'] = 1;
                 $_smarty_tpl->tpl_vars['smarty']->value['section']['n']['iteration'] <= $_smarty_tpl->tpl_vars['smarty']->value['section']['n']['total'];
                 $_smarty_tpl->tpl_vars['smarty']->value['section']['n']['index'] += $_smarty_tpl->tpl_vars['smarty']->value['section']['n']['step'], $_smarty_tpl->tpl_vars['smarty']->value['section']['n']['iteration']++):
$_smarty_tpl->tpl_vars['smarty']->value['section']['n']['rownum'] = $_smarty_tpl->tpl_vars['smarty']->value['section']['n']['iteration'];
$_smarty_tpl->tpl_vars['smarty']->value['section']['n']['index_prev'] = $_smarty_tpl->tpl_vars['smarty']->value['section']['n']['index'] - $_smarty_tpl->tpl_vars['smarty']->value['section']['n']['step'];
$_smarty_tpl->tpl_vars['smarty']->value['section']['n']['index_next'] = $_smarty_tpl->tpl_vars['smarty']->value['section']['n']['index'] + $_smarty_tpl->tpl_vars['smarty']->value['section']['n']['step'];
$_smarty_tpl->tpl_vars['smarty']->value['section']['n']['first']      = ($_smarty_tpl->tpl_vars['smarty']->value['section']['n']['iteration'] == 1);
$_smarty_tpl->tpl_vars['smarty']->value['section']['n']['last']       = ($_smarty_tpl->tpl_vars['smarty']->value['section']['n']['iteration'] == $_smarty_tpl->tpl_vars['smarty']->value['section']['n']['total']);
?>
				<?php if ($_smarty_tpl->tpl_vars['bulletin']->value['list'][$_smarty_tpl->getVariable('smarty')->value['section']['n']['index']]['pic_id']!=''){?>
				<div class="work_one">
                	<img src="<?php echo $_smarty_tpl->tpl_vars['web_url']->value;?>
/common/upload/images/<?php echo $_smarty_tpl->tpl_vars['bulletin']->value['list'][$_smarty_tpl->getVariable('smarty')->value['section']['n']['index']]['pic_link'];?>
" alt="picture" />
                    <div class="work_concent">
                		<div class="work_title">
                    		<p>
                    			<?php if ($_smarty_tpl->tpl_vars['bulletin']->value['list'][$_smarty_tpl->getVariable('smarty')->value['section']['n']['index']]['aa_top']!=''){?>
                				<a style="color:red;" href="<?php echo $_smarty_tpl->tpl_vars['web_url']->value;?>
/index.php/activityjobbulletin/detail/id/<?php echo $_smarty_tpl->tpl_vars['bulletin']->value['list'][$_smarty_tpl->getVariable('smarty')->value['section']['n']['index']]['aa_id'];?>
">
                					[顶]<?php echo $_smarty_tpl->tpl_vars['bulletin']->value['list'][$_smarty_tpl->getVariable('smarty')->value['section']['n']['index']]['aa_title'];?>

                				</a>
                				<?php }else{ ?>
                				<a href="<?php echo $_smarty_tpl->tpl_vars['web_url']->value;?>
/index.php/activityjobbulletin/detail/id/<?php echo $_smarty_tpl->tpl_vars['bulletin']->value['list'][$_smarty_tpl->getVariable('smarty')->value['section']['n']['index']]['aa_id'];?>
">
                					<?php echo $_smarty_tpl->tpl_vars['bulletin']->value['list'][$_smarty_tpl->getVariable('smarty')->value['section']['n']['index']]['aa_title'];?>

                				</a>
                				<?php }?>
                    		</p>
                    		<em><?php echo smarty_modifier_date_format($_smarty_tpl->tpl_vars['bulletin']->value['list'][$_smarty_tpl->getVariable('smarty')->value['section']['n']['index']]['aa_time'],"%Y-%m-%d");?>
</em>
                    		<div style="clear:both;" ></div>
                    	</div>
                        <div class="concent">
                            <p><?php echo smarty_modifier_truncate(preg_replace('!<[^>]*?>!', ' ', $_smarty_tpl->tpl_vars['bulletin']->value['list'][$_smarty_tpl->getVariable('smarty')->value['section']['n']['index']]['aa_content']),130,'…');?>

                            </p>
                         </div>
                    	<div class="work_read">
                        		<div class="work_read_left">阅读次数：<?php echo $_smarty_tpl->tpl_vars['bulletin']->value['list'][$_smarty_tpl->getVariable('smarty')->value['section']['n']['index']]['aa_scan'];?>

                        		&nbsp;&nbsp;分享数：<?php echo $_smarty_tpl->tpl_vars['bulletin']->value['list'][$_smarty_tpl->getVariable('smarty')->value['section']['n']['index']]['aa_share'];?>

                        		</div>
                            	<div class="work_read_right" >
                            		<a href="<?php echo $_smarty_tpl->tpl_vars['web_url']->value;?>
/index.php/activityjobbulletin/detail/id/<?php echo $_smarty_tpl->tpl_vars['bulletin']->value['list'][$_smarty_tpl->getVariable('smarty')->value['section']['n']['index']]['aa_id'];?>
">查看详情>></a>
                        		</div>
                        		<div style="clear:both;" ></div>
                   		</div>
                   </div>
                   <div style="clear:both;" ></div>
                </div>
			
				<?php }else{ ?>
					   
                
                <div class="work_two">
                	<div class="work_titleone">
                		<p>
                			<?php if ($_smarty_tpl->tpl_vars['bulletin']->value['list'][$_smarty_tpl->getVariable('smarty')->value['section']['n']['index']]['aa_top']!=''){?>
                			<a style="color:red;" href="<?php echo $_smarty_tpl->tpl_vars['web_url']->value;?>
/index.php/activityjobbulletin/detail/id/<?php echo $_smarty_tpl->tpl_vars['bulletin']->value['list'][$_smarty_tpl->getVariable('smarty')->value['section']['n']['index']]['aa_id'];?>
">
                			[顶]<?php echo $_smarty_tpl->tpl_vars['bulletin']->value['list'][$_smarty_tpl->getVariable('smarty')->value['section']['n']['index']]['aa_title'];?>

                			</a>
                			<?php }else{ ?>
                			<a href="<?php echo $_smarty_tpl->tpl_vars['web_url']->value;?>
/index.php/activityjobbulletin/detail/id/<?php echo $_smarty_tpl->tpl_vars['bulletin']->value['list'][$_smarty_tpl->getVariable('smarty')->value['section']['n']['index']]['aa_id'];?>
">
                			<?php echo $_smarty_tpl->tpl_vars['bulletin']->value['list'][$_smarty_tpl->getVariable('smarty')->value['section']['n']['index']]['aa_title'];?>

                			</a>
                			<?php }?>
                		</p>
                		<em><?php echo smarty_modifier_date_format($_smarty_tpl->tpl_vars['bulletin']->value['list'][$_smarty_tpl->getVariable('smarty')->value['section']['n']['index']]['aa_time'],"%Y-%m-%d");?>
</em>
 						<div style="clear:both;" ></div>
                    </div>
                    <div class="concentone">
                        <p>
                        	<?php echo smarty_modifier_truncate(preg_replace('!<[^>]*?>!', ' ', $_smarty_tpl->tpl_vars['bulletin']->value['list'][$_smarty_tpl->getVariable('smarty')->value['section']['n']['index']]['aa_content']),130,'…');?>

                        </p>
                    </div>
                    <div class="work_readone">
                   		<div class="work_read_left">阅读次数：<?php echo (($tmp = @$_smarty_tpl->tpl_vars['bulletin']->value['list'][$_smarty_tpl->getVariable('smarty')->value['section']['n']['index']]['aa_scan'])===null||$tmp==='' ? "0" : $tmp);?>

                      		&nbsp;&nbsp;分享数：<?php echo (($tmp = @$_smarty_tpl->tpl_vars['bulletin']->value['list'][$_smarty_tpl->getVariable('smarty')->value['section']['n']['index']]['aa_share'])===null||$tmp==='' ? "0" : $tmp);?>

                    	</div>
                        <div class="work_read_right" >
                          		<a href="<?php echo $_smarty_tpl->tpl_vars['web_url']->value;?>
/index.php/activityjobbulletin/detail/id/<?php echo $_smarty_tpl->tpl_vars['bulletin']->value['list'][$_smarty_tpl->getVariable('smarty')->value['section']['n']['index']]['aa_id'];?>
">查看详情>></a>
                      		</div>
                      		<div style="clear:both;" ></div>
                   	</div>
                   	<div style="clear:both;" ></div>
                </div>
                <?php }?>
					
				<?php endfor; endif; ?>
                
             </div>
     
      		<?php echo smarty_function_page(array('info'=>$_smarty_tpl->tpl_vars['bulletin']->value,'web_url'=>$_smarty_tpl->tpl_vars['web_url']->value,'url'=>"/index.php/activityjobbulletin/index/id/".((string)$_smarty_tpl->tpl_vars['perid']->value['ap_id'])),$_smarty_tpl);?>

     
         </div>   
         	<?php $_smarty_tpl->tpl_vars['tuijianzhaopinhui'] = new Smarty_variable(1, null, 0);?>
         	<?php $_smarty_tpl->tpl_vars['humanid'] = new Smarty_variable('red_human', null, 0);?>
			<?php echo $_smarty_tpl->getSubTemplate ('middle-right.html', $_smarty_tpl->cache_id, $_smarty_tpl->compile_id, null, null, array(), 0);?>

                
    </div>
    <?php echo $_smarty_tpl->getSubTemplate ('footer.htm', $_smarty_tpl->cache_id, $_smarty_tpl->compile_id, null, null, array(), 0);?>
  <?php }} ?>